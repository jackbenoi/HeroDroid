    ...
    <div class="col-xs-12 col-md-12 col-lg-12">
        <div class="card">
            <div class="card-header ">
                <h4 class="card-title">
                    Apps Informations |  <a href="{{ route('backend.apps.market') }}" class="btn btn-sm btn-success"><i class="fa fa-android"></i> Create App/Games from Playstore</a> | <a href="{{ route('backend.apps.detail') }}" class="btn btn-sm bg-dark"> Add Manual App/Games</a>
                </h4>
            </div>
                    ...
                    <table class="table table-bordered table-striped m-b-0">
                        ...
                        <tbody block-ui="blockui-effect">
                            ...
                            <tr class="items ng-cloak" ng-repeat="list in data track by $index">
                                ...
                                <td>
                                    <div uib-tooltip="@{{ list.is_enabled == '1' ? 'Enabled' : 'Disabled'}}">@{{ list.is_enabled == '1' ? 'Enabled' : "Disabled"}}</div>
                                </td>
                                <td align="center">

                                    @if($isDemo)
                                        <button uib-tooltip="Demo Mode On for @{{ list.title }}" class="btn btn-sm btn-success" type="button" ><i class="fa fa-android"></i> Grab Details</button>
                                    
                                    @else
                                         <button uib-tooltip="Grab Other Details for @{{ list.title }}" class="btn btn-sm btn-success" type="button" ng-click="grabDetailAndSave(list.app_id)"><i class="fa fa-android"></i> Grab Details</button>
                                    @endif
